...
/** Routes with light page backgrounds where white nav text would be invisible. */
const LIGHT_ROUTES = ['/projects']

function isLightRoute(pathname) {
  return LIGHT_ROUTES.some(
...
export default function Header() {
  ...
  const lightPage = isLightRoute(location.pathname)

  // On light pages always use solid bar so nav stays readable over light content.
  const solid = scrolled || lightPage
  // Light chrome only on light routes when we want dark ink; solid dark bar uses white text.
  const useLightChrome = lightPage
  ...
  const headerBg = useLightChrome
    ? 'bg-brand-surface/90 backdrop-blur-xl border-b border-brand-ink/10 shadow-sm shadow-brand-ink/5'
    : solid
      ? 'bg-brand-dark/90 backdrop-blur-xl border-b border-white/10 shadow-lg shadow-brand-dark/10'
      : 'bg-transparent'

  const logoClass = useLightChrome
    ? 'text-lg font-display font-bold text-brand-ink'
    : 'text-lg font-display font-bold text-white'

  const navClass = useLightChrome
    ? 'flex gap-6 text-sm font-semibold items-center text-brand-ink/75'
    : 'flex gap-6 text-sm font-semibold items-center text-white/80'

  const navLinkHover = useLightChrome
    ? 'hover:text-brand-ink transition-colors'
    : 'hover:text-white transition-colors'

  const menuButtonClass = useLightChrome
    ? 'p-2 rounded-lg bg-brand-ink/5 text-brand-ink border border-brand-ink/10'
    : 'p-2 rounded-lg bg-white/10 text-white border border-white/15'

  const socialClass = useLightChrome
    ? 'social-btn !text-brand-ink/80 !border-brand-ink/15 !bg-brand-ink/5 hover:!text-brand-ink hover:!border-brand-primary/40'
    : 'social-btn'
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${headerBg}`}>
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          <nav className={navClass}>
            <Link to="/" className={navLinkHover}>Inicio</Link>
            <a href="/#projects" onClick={handleProjectsClick} className={navLinkHover}>Proyectos</a>
            <Link to="/about" className={navLinkHover}>Sobre mi</Link>
            <Link to="/education" className={navLinkHover}>Educacion</Link>
            <Link to="/contact" className={navLinkHover}>Contacto</Link>
          </nav>

          <div className="flex items-center gap-3">
            <a href={SOCIAL_LINKS.github} target="_blank" rel="noreferrer" aria-label="GitHub" className={socialClass}>
            ...
            <a href={SOCIAL_LINKS.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" className={socialClass}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
            ...
            <a href={SOCIAL_LINKS.instagram} target="_blank" rel="noreferrer" aria-label="Instagram" className={socialClass}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
        ...
        <div className="md:hidden flex items-center">
          <button onClick={() => setOpen(o => !o)} className={menuButtonClass} aria-label="Abrir menú">
      ...
      {/* Mobile overlay menu — always dark for contrast */}
